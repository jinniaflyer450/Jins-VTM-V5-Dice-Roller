...
function v5Roll(totalPool, hunger, diff){
    ...
    if(totalResults.success >= diff && totalResults.messyCrit){
        return `Messy Critical--rolled ${totalResults.success} success(es) against a difficulty of ${diff}.`
    }
    else if(totalResults.success >= diff && totalResults.normalCrit){
        return `Normal Critical--rolled ${totalResults.success} success(es) against a difficulty of ${diff}.`
    }
    else if(totalResults.success >= diff){
        return `Success--rolled ${totalResults.success} success(es) against a difficulty of ${diff}.`
    }
    else if(totalResults.success < diff && totalResults.bestial){
        return `Bestial Failure--rolled ${totalResults.success} success(es) against a difficulty of ${diff}.`
    }
    else{
        return `Failure--rolled ${totalResults.success} success(es) against a difficulty of ${diff}.`
    }
}
